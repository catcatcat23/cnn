...
假设输入图像或特征图的尺寸为 $\(W_{\text{in}} \times H_{\text{in}}\)$，卷积核的大小为 $\(F \times F\)$，步长为 \(S\)，填充为 \(P\)，那么卷积后的输出图像尺寸 $\(W_{\text{out}} \times H_{\text{out}}\)$ 可以通过以下公式计算：
...
其中：
- $\(W_{\text{in}}\)$ 和 $\(H_{\text{in}}\)$ 是输入特征图的宽度和高度。
- \(F\) 是卷积核的大小（通常为正方形，即 \(F \times F\)）。
- \(S\) 是步长（stride）。
- \(P\) 是填充的大小，通常为零填充（zero padding）。

## 示例计算

### 示例 1：没有填充（Valid Padding）

假设输入图像大小为 $\(32 \times 32\)$，卷积核大小为 $\(3 \times 3\)$，步长 \(S = 1\)，填充 \(P = 0\)，则卷积后的输出尺寸为：

$$
W_{\text{out}} = \frac{32 - 3 + 2(0)}{1} + 1 = 30
$$

$$
H_{\text{out}} = \frac{32 - 3 + 2(0)}{1} + 1 = 30
$$

所以，卷积后的输出特征图大小为 $\(30 \times 30\)$。

### 示例 2：使用填充（Same Padding）

如果使用填充，假设输入图像大小为 $\(32 \times 32\)$，卷积核大小为 $\(3 \times 3\)$，步长 \(S = 1\)，填充 \(P = 1\)，则卷积后的输出尺寸为：
...
$$
H_{\text{out}} = \frac{32 - 3 + 2(1)}{1} + 1 = 32
$$

所以，卷积后的输出特征图大小为 $\(32 \times 32\)$，即与输入图像尺寸相同。
